...
CSRF_RESPONSE=$(curl -s "$API_BASE/admin/csrf")
...
CREATE_RESPONSE=$(curl -s -X POST "$API_BASE/admin/albums" \
...
UPDATE_RESPONSE=$(curl -s -X PATCH "$API_BASE/admin/albums/$ALBUM_ID" \
    -H "Content-Type: application/json" \
    -H "x-csrf-token: $CSRF_TOKEN" \
    -d '{
...
UPLOAD_RESPONSE=$(curl -s -X POST "$API_BASE/admin/albums/$ALBUM_ID/photos" \
    -H "x-csrf-token: $CSRF_TOKEN" \
...
if [ -n "$PHOTO_ID" ] && [ "$PHOTO_ID" != "null" ]; then
    DELETE_PHOTO_RESPONSE=$(curl -s -X DELETE "$API_BASE/admin/albums/$ALBUM_ID/photos" \
        -H "Content-Type: application/json" \
        -H "x-csrf-token: $CSRF_TOKEN" \
        -d "{\"photoId\": \"$PHOTO_ID\"}")
...
DELETE_ALBUM_RESPONSE=$(curl -s -X DELETE "$API_BASE/admin/albums/$ALBUM_ID" \
    -H "x-csrf-token: $CSRF_TOKEN")
...
ALBUMS_RESPONSE=$(curl -s "$API_BASE/admin/albums")
...
if [ -z "$ALBUM_STILL_EXISTS" ]; then
    ...
    test_result 1 "Album no longer exists in list" "Album still found: $ALBUM_STILL_EXISTS"
fi
echo ""
